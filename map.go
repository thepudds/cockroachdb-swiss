...
// slots. The mirrored control bytes are no longer needed and we always probe
// a group's entire set of control bytes.
//
...
type bucket[K comparable, V any] struct {
	...
	// The mask used to compute the index of the group within the bucket. The
	// groupMask only changes when a bucket is resized.
	groupMask uint32
...
func (m *Map[K, V]) Init(initialCapacity int, options ...Option[K, V]) {
	*m = Map[K, V]{
		...
		bucket0: bucket[K, V]{
			...
			// the bucket.
			groups: makeUnsafeSlice(unsafeConvertSlice[Group[K, V]](emptyGroup)),
		},
		...
		for match != 0 {
			...
			if key == slot.key {
				...
				*slot = Slot[K, V]{}
				...
				// can prove that the slot would not appear in the middle of a
				// probe sequence we can mark the slot as empty instead. We
				// can prove this by checking to see if the group contains an
				// empty slot which terminates a probe sequence (assuming we
				// never create an empty slot in a group with no empties which
				// this heuristic guarantees we never do).
...
func (m *Map[K, V]) buckets(offset uintptr, yield func(b *bucket[K, V]) bool) {
	...
	startBucket := m.dir.At(offset & uintptr(m.bucketCount()-1))
	for b := startBucket; ; {
...
func (b *bucket[K, V]) rehashInPlace(m *Map[K, V]) {
	...
	for i := uint32(0); i <= b.groupMask; i++ {
		...
		for j := uint32(0); j < groupSize; j++ {
			...
			if targetGroup.ctrls.Get(target) == ctrlEmpty {
				...
				*targetGroup.slots.At(target) = *slot
				*slot = Slot[K, V]{}
				g.ctrls.Set(j, ctrlEmpty)
				continue
			}

			if targetGroup.ctrls.Get(target) == ctrlDeleted {
...
func unsafeCtrlGroup(ctrls []ctrl) *ctrlGroup {
	return (*ctrlGroup)(unsafe.Pointer(unsafe.SliceData(ctrls)))
}
...
// emptyGroup is a singleton slice for a single empty group. Because the slots
// are never accessed this singleton can be used for all Group[K,V] types.
var emptyGroup = func() []Group[struct{}, struct{}] {
	var g [1]Group[struct{}, struct{}]
	for i := uint32(0); i < groupSize; i++ {
		g[0].ctrls.Set(i, ctrlEmpty)
	}
	return g[:1]
}()
